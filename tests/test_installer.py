...
def test_pip_install_queues_and_reports_running():
    with patch("installer.subprocess.Popen", return_value=MagicMock(pid=4242)), \
         patch("installer.shutil.which", return_value="/usr/bin/python3"):
        installer.start_pip_install("foo", "foo-mcp-server==1.0", "foo-mcp")

    with patch("installer._pid_running", return_value=True):
...
def test_get_install_status_failed_when_process_disappears_without_exitcode():
    ...
    with patch("installer._pid_running", return_value=False):
        status = installer.get_install_status("foo")
    assert status["status"] == "failed"
...
def test_docker_available_false_when_docker_ps_fails():
    with patch("installer.shutil.which", return_value="/usr/bin/docker"), \
         patch("installer.subprocess.run", return_value=MagicMock(returncode=1)):
        assert installer.docker_available() is False


def test_docker_available_false_when_docker_not_on_path():
    with patch("installer.shutil.which", return_value=None):
        assert installer.docker_available() is False


def test_docker_available_true_when_docker_ps_succeeds():
    with patch("installer.shutil.which", return_value="/usr/bin/docker"), \
         patch("installer.subprocess.run", return_value=MagicMock(returncode=0)):
...
def test_start_docker_pull_does_not_gate_itself(tmp_path):
    ...
    with patch("installer.subprocess.Popen", return_value=MagicMock(pid=1)) as mock_popen:
...
def test_uninstall_removes_install_path_for_pip(tmp_path):
    ...
    with patch("installer.STATE_DIR", tmp_path / "installs"):
...
def test_double_install_blocked_while_still_running():
    with patch("installer.subprocess.Popen", return_value=MagicMock(pid=1)):
